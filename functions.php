...
{	
	add_option(MYD_OPTIONS404);
	$options = array();
	$options['myt_404_redirect_to']= site_url();
	$options['myt_404_status']= '1';	
	update_option(MYD_OPTIONS404,$options);
} 
...
{	
	$options=get_option(MYD_OPTIONS404);
	if(!$options)
	{
		init_my_options();
		$options=get_option(MYD_OPTIONS404);
